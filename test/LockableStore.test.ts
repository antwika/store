...
import { ILockableStore } from '../src/ILockableStore';
import { IStore } from '../src/IStore';
import { LockableStore, Ticket } from '../src/LockableStore';
import { MemoryStore } from '../src/MemoryStore';

type TestResource = { foo: string };

describe('LockableStore', () => {
  let ticketStore: IStore;
  let lockableStore: ILockableStore;
  let store: IStore;
  ...
  beforeEach(() => {
    ...
    lockableStore = new LockableStore(logger, lock, ticketStore, store);
  });
  ...
  describe('createWithoutId', () => {
    it('throws if provided an invalid ticket', async () => {
      await expect(() => lockableStore.createWithoutId<TestResource>('invalid-ticket', { foo: 'bar' })).rejects.toThrowError('Invalid ticket');
    });

    it('throws if a READ ticket is used', async () => {
      const testResource = { foo: 'bar' };
      const ticketId = await lockableStore.acquireTicket('READ');
      await expect(() => lockableStore.createWithoutId<TestResource>(ticketId, testResource)).rejects.toThrowError('Invalid ticket');
    });

    it('can create a new resource in a store after acquiring a WRITE ticket', async () => {
      const testResource = { foo: 'bar' };
      const ticket = await lockableStore.acquireTicket('WRITE');
      await expect(lockableStore.createWithoutId<TestResource>(ticket, testResource))
  ...
  describe('read', () => {
    it('throws if provided an invalid ticket', async () => {
      await expect(() => lockableStore.read<TestResource>('invalid-ticket', 'an id')).rejects.toThrowError('Invalid ticket');
    });

    it('can read an existing resource from a store after acquiring a READ ticket', async () => {
      const testResource = { foo: 'bar' };

      const { id } = await store.createWithoutId<TestResource>(testResource);

      const ticket = await lockableStore.acquireTicket('READ');
      await expect(lockableStore.read<TestResource>(ticket, id))
        .resolves.toStrictEqual({
          ...testResource,
          id: expect.any(String),
        });
  ...
  describe('readAll', () => {
    it('throws if provided an invalid ticket', async () => {
      await expect(() => lockableStore.readAll<TestResource>('invalid-ticket')).rejects.toThrowError('Invalid ticket');
    });

    it('can read all resources from a store after acquiring a READ ticket', async () => {
      const testResource = { foo: 'bar' };

      await store.createWithoutId<TestResource>(testResource);

      const ticket = await lockableStore.acquireTicket('READ');
      await expect(lockableStore.readAll<TestResource>(ticket))
        .resolves.toStrictEqual([{
          ...testResource,
          id: expect.any(String),
        }]);
  ...
  describe('update', () => {
    it('throws if provided an invalid ticket', async () => {
      await expect(() => lockableStore.update<TestResource>('invalid-ticket', { id: 'an id', foo: 'baz' })).rejects.toThrowError('Invalid ticket');
    });
    ...
    it('throws if a READ ticket is used', async () => {
      ...
      const ticketId = await lockableStore.acquireTicket('READ');
      await expect(() => lockableStore.update<TestResource>(ticketId, { id, foo: 'baz' })).rejects.toThrowError('Invalid ticket');
    });
    ...
    it('can update an existing resource in a store after acquiring a WRITE ticket', async () => {
      ...
      const ticket = await lockableStore.acquireTicket('WRITE');
      await expect(lockableStore.update<TestResource>(ticket, { id, foo: 'baz' })).resolves.toBeUndefined();
      await expect(lockableStore.read<TestResource>(ticket, id))
        .resolves.toStrictEqual({
          foo: 'baz',
          id: expect.any(String),
  ...
  describe('delete', () => {
    it('throws if provided an invalid ticket', async () => {
      await expect(() => lockableStore.delete('invalid-ticket', 'an id')).rejects.toThrowError('Invalid ticket');
    });
    ...
    it('throws if a READ ticket is used', async () => {
      ...
      const ticketId = await lockableStore.acquireTicket('READ');
      await expect(() => lockableStore.delete(ticketId, id)).rejects.toThrowError('Invalid ticket');
    });
    ...
    it('can delete an existing resource from a store after acquiring a WRITE ticket', async () => {
      ...
      const ticket = await lockableStore.acquireTicket('WRITE');
      await expect(lockableStore.delete(ticket, id)).resolves.toBeUndefined();
      await expect(lockableStore.read<TestResource>(ticket, id)).rejects.toThrowError('Could not find data by id');
      await expect(lockableStore.returnTicket(ticket)).resolves.toBeUndefined();
    });
  });
});
